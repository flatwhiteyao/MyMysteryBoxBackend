...
export class BlindBoxController {
  ...
  public async searchBlindBoxes(@Query('keyword') keyword: string) {
    try {
      ...
      const blindBoxes = await this.blindBoxService.searchBlindBoxes(
        searchKeyword
      );
  ...
  public async addBlindBox(ctx: Context) {
    try {
      ...
      for (let i = 0; i < styleNames.length; i++) {
        ...
        const styleProbability = parseFloat(
          styleProbabilities[i]?.toString() || '0'
        );
  ...
  public async deleteBlindBox(@Query('id') id: number, ctx: Context) {
    try {
      ...
      const blindBox = await db.get(
        'SELECT photo FROM blind_boxes WHERE id =?',
        [id]
      );
  ...
  public async randomDraw(
    ...
    if (result) {
      await this.userDrawnBlindBoxService.addUserDrawnBlindBox(
        user_id,
        id,
        result.id
      ); // 记录抽中信息
