...
export class BlindBoxStyleService {
  async addBlindBoxStyles(
    blindBoxId: number,
    styles: { name: string; photo: string; probability: number }[]
  ) {
  ...
  async getBlindBoxStyles(blindBoxId: number) {
    ...
    const styles = await db.all(
      'SELECT * FROM blind_box_styles WHERE blind_box_id =?',
      [blindBoxId]
    );
    return styles;
  }
  ...
  async randomDraw(blindBoxId: number) {
    ...
    const totalProbability = styles.reduce(
